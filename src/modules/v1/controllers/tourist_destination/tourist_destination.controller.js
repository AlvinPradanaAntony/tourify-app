...
export const storeTourDestination = async (req, res) => {
    try {
        ...
        const data = req.body.data.map((val, key) => {
            const { tour_category_id, name, phone, email, price, description, address, longitude, latitude, day_open_start, day_open_end, time_open_start, time_open_end, user_id, picture } = val
